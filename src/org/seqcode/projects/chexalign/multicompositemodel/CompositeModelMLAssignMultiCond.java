...
public class CompositeModelMLAssignMultiCond {
	 ...
	 * @throws Exception 
	 */
	public CompositeModelMLAssignMultiCond(CompositeTagDistribution composite, ProteinDNAInteractionModelMultiCond model, XLAnalysisConfig config, ExperimentManager eMan){
    ...
    public CompositeModelSiteAssignmentMultiCond  assignToPointFromComposite(int pointIndex, boolean updatePi
        ...
        for (int c=0; c < numConditions ; c++){
    		...
    		compResp[c] = new double[nonZeroComponents.get(c).size()];
        }
    	for(int c=0; c<numConditions;c++){
    		int x=0;
